...
import { DEFENSE_ROLE_OPTIONS, DEFENSE_ROUNDS, DefenseRound, RoleDefenseState } from '../domain/role-defense.models';
import { TargetPoint } from '../../field-challenge/domain/field-challenge.models';
import { HttpErrorService } from '@core/services/http-error.service';
import { NotificationService } from '@core/services/notification.service';
import { CompleteTrainingGameResponse } from '@shared/models/training.model';
import { RoleDefenseFacade } from '../data-access/role-defense.facade';

interface DefenseToken {
  id: number;
  label: string;
  role: RotationRole;
  zone: number;
...
@Component({
  selector: 'app-role-defense',
  standalone: true,
  imports: [CommonModule, RouterLink, NotConfiguredComponent],
  templateUrl: './role-defense.component.html',
  styleUrls: ['./role-defense.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [RoleDefenseFacade],
})
export class RoleDefenseComponent implements OnInit, OnDestroy {
  private readonly facade = inject(RoleDefenseFacade);
  ...
  state = signal<RoleDefenseState>('setup');
  selectedRoleId = signal<number | null>(null);
  roundIndex = signal(0);
  token = signal<DefenseToken | null>(null);
  ...
  lockedTokens = computed<LockedDefenseToken[]>(() => {
    const selectedId = this.selectedRoleId();
    if (selectedId === null) return [];

    const round = this.currentRound();
    return round.rotation.players
      ...
      .map((player) => ({
        ...
        role: player.role,
        zone: player.zone,
        point: this.resolveTargetPoint(round, player),
      }));
  });
  ...
  private loadRound(): void {
    ...
    const round = this.currentRound();
    const player = round.rotation.players.find((entry) => entry.id === selected.id);
    if (!player) return;

    const targetPoint = this.resolveTargetPoint(round, player);
    this.token.set({
      id: player.id,
      label: player.label,
      role: player.role,
  ...
  onDragStart(event: DragEvent): void {
    ...
    event.dataTransfer?.setData('text/plain', 'defense-token');
  }
  ...
  resultTitle(): string {
    ...
    if (score >= 500) return 'Defensa top';
    if (score >= 360) return 'Muy buen trabajo';
    if (score >= 240) return 'Buen progreso';
    return 'Sigue practicando';
  }

  private resolveTargetPoint(round: DefenseRound, player: { role: RotationRole; zone: number }): TargetPoint {
    const slot = this.getDefenseSlot(player);
    return round.scenario.targets[slot];
  }

  private getDefenseSlot(player: { role: RotationRole; zone: number }): 1 | 2 | 3 | 4 | 5 | 6 {
    const front = this.isFront(player.zone);
      ...
      .subscribe({
        next: (result) => {
          ...
          if (result.awardedPoints > 0) {
            this.notifications.success(`+${result.awardedPoints} puntos en ranking (Defensa por rol).`);
          } else {
        ...
        error: (err) => {
          this.notifications.error(
            this.httpErrors.getMessage(err, 'No se pudo registrar la puntuacion de defensa por rol.'),
          );
        },
      });
  }
